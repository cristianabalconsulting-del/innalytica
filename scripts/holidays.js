// holidays.js — Festividades por pais/region desde dos fuentes gratis:
...
// Devuelve [{d:'YYYY-MM-DD', city:'', name, festivo:'Si'}] de los proximos 180 dias.
const CAL_KEY = process.env.CALENDARIFIC_KEY || '';
const sleep = (ms) => new Promise(r => setTimeout(r, ms));
const nagerCache = {};   // year|ISO -> [{date,name,global}]
const calCache = {};     // year|ISO -> [{date,name,states}]
  ...
  for (const iso of Object.keys(isos)){
    ...
    for (const y of ys){
      ...
      nag.forEach(function(h){
        if (h.date < start || h.date > end) return;
        if (h.global === false) return;   // solo nacionales (los regionales los da Calendarific)
